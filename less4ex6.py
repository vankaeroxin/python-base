...
def load_process(cyc):
    """
    Функция имитирует процесс загрузки
    На экран последовательно выводится строка из 10 точек, которые замещаются символом #
    Принимает на вход целое число - количество повторений процесса загрузки

    Выводится список, сгенерированный при помощи itertools.cycle()
